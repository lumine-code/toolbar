...
module.exports = {
  provideBackgroundTips() {
    return {
      packageName: "toolbar",
      tips: ["You can show or hide the toolbar with {{ 'toolbar:toggle' | keystroke }}"],
    };
  },
  ...
  activate() {
    ...
    this.subscriptions = new CompositeDisposable(
      lumine.commands.add("lumine-workspace", {
        "toolbar:toggle": () => this.ensureView().toggle(),
        "toolbar:position-top": {
          description: "Put the toolbar along the top of the window.",
          didDispatch: () => this.ensureView().setPosition("Top"),
        },
        "toolbar:position-right": {
          description: "Put the toolbar down the right of the window.",
          didDispatch: () => this.ensureView().setPosition("Right"),
        },
        "toolbar:position-bottom": {
          description: "Put the toolbar along the bottom of the window.",
          didDispatch: () => this.ensureView().setPosition("Bottom"),
        },
        "toolbar:position-left": {
          description: "Put the toolbar down the left of the window.",
          didDispatch: () => this.ensureView().setPosition("Left"),
        },
      }),
    );

    // Keep the default-visible toolbar behavior without charging its DOM and
    // panel construction to package activation.  A consumer that requests the
    // service or a command dispatched immediately after activation still uses
    // ensureView() as a synchronous fallback.
    queueMicrotask(() => {
      if (this.activationGeneration === generation) this.ensureView();
    });
  },

  ensureView() {
    return (this.view ??= new ToolbarView());
  },
  ...
  provideToolbar() {
    return (group) => new ToolbarManager(group, this.ensureView());
  },
};
